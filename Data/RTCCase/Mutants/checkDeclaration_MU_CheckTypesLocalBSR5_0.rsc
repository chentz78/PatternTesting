...
@bootstrapParser 
module checkDeclaration_MU_CheckTypesLocalBSR5_0
...
public Configuration checkDeclaration(Declaration decl:(Declaration)`<Tags tags> <Visibility vis> <Type t> <{Variable ","}+ vars>;`, bool descend, Configuration c) {
  println("checkDeclaration:5859");
	...
	if (!descend) {
	    for (v <- vars, v@\loc notin c.definitions<1>, v@\loc notin {l | error(_,l) <- c.messages}) throw "mutant! BSR5: Remove for conditionals.";	
	} else {
