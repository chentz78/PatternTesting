...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR1_18
...
public CheckResult checkStmt(Statement stmt:(Statement)`<Label lbl> for ( <{Expression ","}+ gens> ) <Statement bdy>`, Configuration c) {
  println("checkStmt:4576");
    ...
    if ((Label)`<Name n> :` := lbl) {
        labelName = convertName(n);
        if (labelExists(cFor,labelName)) 
  throw "mutant! BSR1: Remove if conditionals. Case 1.";cFor = addLabel(cFor,labelName,lbl@\loc,forLabel());
