...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR3_12
...
public CheckResult checkStmt(Statement stmt:(Statement)`<Label lbl> for ( <{Expression ","}+ gens> ) <Statement bdy>`, Configuration c) {
  println("checkStmt:4576");
    ...
    if (size(failures) > 0) return markLocationFailed(c, stmt@\loc, failures); else throw "mutant! BSR3: Remove if conditionals. Case 3."; 
}
