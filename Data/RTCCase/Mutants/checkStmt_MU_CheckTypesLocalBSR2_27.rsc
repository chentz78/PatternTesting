...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR2_27
...
public CheckResult checkStmt(Statement stmt:(Statement)`insert <DataTarget dt> <Statement s>`, Configuration c) {
  println("checkStmt:5030");
    ...
    if ((DataTarget)`<Name n>:` := dt) {
        labelName = convertName(n);
        // TODO: Check to see what category the label is in?
        if (labelName notin c.labelEnv) throw "mutant! BSR2: Remove if conditionals. Case 2."; else if (visitLabel() !:= c.store[c.labelEnv[labelName]].source) {
