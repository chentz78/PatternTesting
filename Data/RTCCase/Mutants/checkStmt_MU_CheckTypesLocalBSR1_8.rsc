...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR1_8
...
public CheckResult checkStmt(Statement stmt:(Statement)`<Label lbl> while ( <{Expression ","}+ conds> ) <Statement bdy>`, Configuration c) {
  println("checkStmt:4428");
    ...
    if ((Label)`<Name n> :` := lbl) {
        labelName = convertName(n);
        if (labelExists(cWhile,labelName)) 
  throw "mutant! BSR1: Remove if conditionals. Case 1.";cWhile = addLabel(cWhile,labelName,lbl@\loc,whileLabel());
