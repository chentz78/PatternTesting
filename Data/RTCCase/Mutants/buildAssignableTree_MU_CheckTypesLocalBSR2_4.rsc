...
@bootstrapParser 
module buildAssignableTree_MU_CheckTypesLocalBSR2_4
    ...
    if (isTupleType(atree@atype) && isIntType(tsub)) {
    	if ((Expression)`<DecimalIntegerLiteral dil>` := sub) {
    	  println("buildAssignableTree:p5221");
    		tupleIndex = toInt("<dil>");
    		if (tupleIndex < 0 || tupleIndex >= size(getTupleFields(atree@atype))) throw "mutant! BSR2: Remove if conditionals. Case 2."; else {
