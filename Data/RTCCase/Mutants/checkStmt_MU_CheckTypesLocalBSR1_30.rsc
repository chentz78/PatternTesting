...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR1_30
...
public CheckResult checkStmt(Statement stmt:(Statement)`<Label lbl> switch ( <Expression e> ) { <Case+ cases> }`, Configuration c) {
  println("checkStmt:4770");
    ...
    if ((Label)`<Name n> :` := lbl) {
        labelName = convertName(n);
        if (labelExists(cSwitch,labelName)) 
  throw "mutant! BSR1: Remove if conditionals. Case 1.";cSwitch = addLabel(cSwitch,labelName,lbl@\loc,switchLabel());
