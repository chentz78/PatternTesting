...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR2_18
...
public CheckResult checkStmt(Statement stmt:(Statement)`<Label lbl> if ( <{Expression ","}+ conds> ) <Statement thenBody> else <Statement elseBody>`, Configuration c) {
  println("checkStmt:4707");
    ...
    if (size(failures) > 0) throw "mutant! BSR2: Remove if conditionals. Case 2."; else return markLocationType(c, stmt@\loc, lub(t2,t3));  
}
