...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR3_0
...
public CheckResult checkStmt(Statement stmt:(Statement)`assert <Expression e>;`, Configuration c) {
    < c, t1 > = checkExp(e, c);
    if (isFailType(t1)) return markLocationFailed(c, stmt@\loc, t1); else throw "mutant! BSR3: Remove if conditionals. Case 3.";return markLocationType(c, stmt@\loc, Symbol::\bool());
}
