...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR1_13
...
public CheckResult checkStmt(Statement stmt:(Statement)`<Label lbl> do <Statement bdy> while (<Expression cond>);`, Configuration c) {
  println("checkStmt:4505");
    ...
    if ((Label)`<Name n> :` := lbl) {
        labelName = convertName(n);
        if (labelExists(cDoWhile,labelName)) 
  throw "mutant! BSR1: Remove if conditionals. Case 1.";cDoWhile = addLabel(cDoWhile,labelName,lbl@\loc,doWhileLabel());
