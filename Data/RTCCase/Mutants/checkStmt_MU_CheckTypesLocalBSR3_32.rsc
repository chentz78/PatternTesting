...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR3_32
...
public CheckResult checkStmt(Statement stmt:(Statement)`append <DataTarget dt> <Statement s>`, Configuration c) {
  println("checkStmt:5067");
    ...
    if ((DataTarget)`<Name n>:` := dt) {
        rn = convertName(n);
        // TODO: Check to see what category the label is in?
        if (rn notin c.labelEnv) failures += makeFailType("Target label not defined", dt@\loc); else throw "mutant! BSR3: Remove if conditionals. Case 3.";
    } else {
