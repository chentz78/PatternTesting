...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR2_12
...
public CheckResult checkStmt(Statement stmt:(Statement)`<Label lbl> for ( <{Expression ","}+ gens> ) <Statement bdy>`, Configuration c) {
  println("checkStmt:4576");
    ...
    if (size(failures) > 0) throw "mutant! BSR2: Remove if conditionals. Case 2."; else return markLocationType(c, stmt@\loc, makeListType(loopElementType)); 
}
