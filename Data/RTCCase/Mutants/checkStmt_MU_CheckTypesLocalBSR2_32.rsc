...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR2_32
...
public CheckResult checkStmt(Statement stmt:(Statement)`append <DataTarget dt> <Statement s>`, Configuration c) {
  println("checkStmt:5067");
    ...
    if ((DataTarget)`<Name n>:` := dt) {
        rn = convertName(n);
        // TODO: Check to see what category the label is in?
        if (rn notin c.labelEnv) throw "mutant! BSR2: Remove if conditionals. Case 2."; else c = addAppendTypeInfo(c, t1, rn, { forLabel(), whileLabel(), doWhileLabel() }, stmt@\loc);
    } else {
