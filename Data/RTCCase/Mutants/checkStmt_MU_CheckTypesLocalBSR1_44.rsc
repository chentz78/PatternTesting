...
@bootstrapParser 
module checkStmt_MU_CheckTypesLocalBSR1_44
...
public CheckResult checkStmt(Statement stmt:(Statement)`<Label lbl> { <Statement+ stmts> }`, Configuration c) {
  println("checkStmt:4948");
    ...
    if ((Label)`<Name n> :` := lbl) {
        labelName = convertName(n);
        if (labelExists(cBlock,labelName)) 
  throw "mutant! BSR1: Remove if conditionals. Case 1.";cBlock = addLabel(cBlock,labelName,lbl@\loc,blockLabel());
