@doc{
Module adapted from Davy Landman implementation. http://homepages.cwi.nl/~landman/icsme2014/
}
...
int getSLOCLines(loc l) {
	...
	if ((Lines)`<Statement* st>` := t.top) {
		return size([ *{ s@\loc.begin.line | Statement s <- st} ]);
	}
